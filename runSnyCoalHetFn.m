...
function [x, xhat, Nhatmean, Nhatlb, Nhatub, nLin, tLin, sampSny, tN, Nt] = runSnyCoalHetFn(M, n, fnid, mi, nSampTimes)
% Usage:
...
for ii = 1:M
    ...
    for i = 1:numRV
        ...
        wt = qmarg{ii}{i};
        if any(wt < 0)
            disp('Weights not all non-negative');
            disp(wt(wt < 0));
            wt(wt < 0) = 0;
        end
        sampSny(id1:id2, i) = datasample(xset{i}, nSamps, 'Weights', wt);
    end
end
...
if M > 1
    ...
    saveas(gcf, ['sampErr' num2str(nSampTimes) '.fig'])
    ...
    saveas(gcf, ['sampBox' num2str(nSampTimes) '.fig'])
...
if M > 1
    ...
    title([fnname ': [n, m, M] = [' [num2str(n) ' ' num2str(m) ' ' num2str(M)] ']']);
    saveas(gcf, ['sampDemo' num2str(nSampTimes) '.fig'])
else
...
saveas(gcf, ['sampPost' num2str(nSampTimes) '.fig']);
...
%vlist = {'qev', 'tset', 'Nhat', 'NhatSq', 'NhatFn', 'NhatFnSq', 'tree', 'sampSny', 'qnlast', 'xhatEv', 'xsetRow', 'IDMx'};
